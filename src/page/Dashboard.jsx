...
const Dashboard = () => {
  ...
  const fetchPokemons = useCallback(async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/pokemons/list`);
      if (res.status === 200) {
  ...
  const fetchComments = async (pokemonId) => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/comments/${pokemonId}`);
  ...
  const handleCommentSubmit = async (pokemonId) => {
    ...
    try {
      await axios.post(`${import.meta.env.VITE_API_URL}/comments/add`, {
        pokemonId,
  ...
  const longPollComments = useCallback(async (pokemonId) => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/comments/longpoll/${pokemonId}`);
      if (res.status !== 204) {
        setComments((prev) => ({
          ...prev,
  ...
  useEffect(() => {
    const intervalId = setInterval(fetchPokemons, 5000);
    fetchPokemons(); // Ensure the initial fetch
    return () => clearInterval(intervalId);
  }, [fetchPokemons]);

  useEffect(() => {
    pokemons.forEach((pokemon) => {
      longPollComments(pokemon._id);
    });
          ...
          <div key={pokemon._id} className="bg-gray-800 p-4 rounded-lg shadow-md">
            ...
            <div>
              ...
              <div className="max-h-32 overflow-y-auto">
                {Array.isArray(comments[pokemon._id]) ? comments[pokemon._id].map((comment) => (
